...
QcValuesItem *QcGaugeWidget::addValues(float position, int highPres)
{
    QcValuesItem * item = new QcValuesItem(this, highPres);
    item->setPosition(position);
    mItems.append(item);
    return item;
}
...
QcColorBand *QcGaugeWidget::addColorBand(float position, int highPres)
{
    QcColorBand * item = new QcColorBand(this, highPres);
    item->setPosition(position);
    mItems.append(item);
    return item;
}
...
QcColorBand::QcColorBand(QObject *parent, int highPres) :
    QcScaleItem(parent)
{
    QColor tmpColor;
...
QcValuesItem::QcValuesItem(QObject *parent, int highPres) :
    QcScaleItem(parent)
{
    setPosition(70);
    mColor = Qt::black;
    mStep = highPres ? 500 : 250;
}
...
void QcValuesItem::draw(QPainter*painter)
{
    ...
    QFont font("Meiryo UI",0, QFont::Normal);
